...
from ..nlp import rag_tokenizer, term_weight, synonym
from app.infrastructure.vector_store.base import MatchTextExpr


class FulltextQueryer:
    """
    全文检索查询器，用于构建复杂的文本搜索查询
    支持中英文混合查询、同义词扩展、词权重计算等功能
    """
    ...
    def subSpecialChar(line):
        """
        转义特殊字符，防止在Elasticsearch查询中出现语法错误
    ...
    def rmWWW(txt):
        """
        移除文本中的疑问词、停用词等无意义词汇，提取核心关键词
        
        入参:
            txt (str): 原始文本
            
        出参:
            str: 清理后的文本，如果清理后为空则返回原文本
        """
        patts = [
            (
    ...
    def question(self, txt, tbl="qa", min_match: float = 0.6):
        ...
        出参:
            tuple: (MatchTextExpr对象, 关键词列表) 或 (None, 关键词列表)
        """
    ...
    def hybrid_similarity(self, avec, bvecs, atks, btkss, tkweight=0.3, vtweight=0.7):
        """
        计算混合相似度，结合向量相似度和词汇相似度
        
        入参:
            avec (array): 查询向量
            bvecs (array): 候选向量列表
            atks (list): 查询文本的词汇列表
            btkss (list): 候选文本的词汇列表
            tkweight (float): 词汇权重，默认为0.3
            vtweight (float): 向量权重，默认为0.7
            
        出参:
            tuple: (混合相似度数组, 词汇相似度列表, 向量相似度数组)
        """
    ...
    def similarity(self, qtwt, dtwt):
        ...
        入参:
            qtwt (dict|str): 查询文本的词汇权重字典或文本字符串
            dtwt (dict|str): 文档文本的词汇权重字典或文本字符串
    ...
    def paragraph(self, content_tks: str, keywords: list = [], keywords_topn=30):
        """
        为段落内容构建查询表达式，提取关键词并生成搜索查询
        
        入参:
            content_tks (str): 段落内容的词汇字符串
            keywords (list): 已有的关键词列表，默认为空
            keywords_topn (int): 提取关键词的最大数量，默认为30
            
        出参:
            MatchTextExpr: 构建的文本匹配表达式对象
        """
