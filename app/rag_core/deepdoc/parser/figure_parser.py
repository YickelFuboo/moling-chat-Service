...
class VisionFigureParser:
    ...
    async def __call__(self, **kwargs):
        ...
        async def process(figure_idx, figure_binary):
            try:
                ...
                callback(-1, f"Figure {figure_idx} processing timeout")
                return figure_idx, ""
            except Exception as e:
                callback(-1, f"Figure {figure_idx} processing error: {e}")
                return figure_idx, ""

        import asyncio
        tasks = []
        for idx, img_binary in enumerate(self.figures or []):
            tasks.append(process(idx, img_binary))

        results = await asyncio.gather(*tasks, return_exceptions=True)
        
        for result in results:
            if isinstance(result, Exception):
                callback(-1, f"Figure processing error: {result}")
                continue
            figure_num, txt = result
            if txt:
